...
def main():
    config_path = Path(os.getenv("TRAINING_CONFIG", "/app/configs/training_config.yaml"))
    if not config_path.exists():
        config_path = Path("configs/training_config.yaml")

    config = load_config(str(config_path))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(json.dumps({"event": "startup", "device": str(device), "config": str(config_path)}), flush=True)
